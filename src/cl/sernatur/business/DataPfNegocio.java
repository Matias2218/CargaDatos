...
import cl.sernatur.service.impl.BasePfServiceImpl;
...
public class DataPfNegocio {
	...
	private BasePf base;
	private BasePfService baseService;
	private Carga carga;
	...
	private void eliminarRegistros() throws Exception {
		baseService = new BasePfServiceImpl(con);
		try {
			String desde = formatoFecha[0];
			String hasta = formatoFecha[1];
	...
	private boolean validartDatos(String[] data, long cuenta) throws Exception {
		...
		try {
			...
			// Validar tipo de dato total
			} else if (!Util.esNumero(total)) {
				Util.setLog(archivoLog, "I", "Insertar Registros --> ERROR en línea " + cuenta, "tipo invalido de total");
				bitacora.add("213;" + cuenta + ";total;" + total + ";Tipo invalido de total, línea " + cuenta);
				...
				base.setTotal(Integer.parseInt(total));
	...
	private void insertarCarga(long cuenta, int estado, int error) throws Exception {
		...
		try {
			...
			carga.setId_base(17); //PF
			carga.setId_estado_carga(estado);
			carga.setId_error_carga(error);
			carga.setNum_rows(cuenta);
			carga.setNum_cols(Integer.parseInt(prop.getProperty("carga.pf.col")));
