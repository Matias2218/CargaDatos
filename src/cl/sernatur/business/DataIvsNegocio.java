...
import cl.sernatur.service.impl.BaseIvsServiceImpl;
...
public class DataIvsNegocio {
	...
	private BaseIvs base;
	private BaseIvsService baseService;
	private Carga carga;
	...
	private void eliminarRegistros() throws Exception {
		baseService = new BaseIvsServiceImpl(con);
		try {
			String desde = formatoFecha[0] + formatoFecha[1];
			String hasta = formatoFecha[2] + formatoFecha[3];
	...
	private boolean validartDatos(String[] data, long cuenta) throws Exception {
		boolean retorno = false;
		try {
			int numeroCampos = Integer.parseInt(prop.getProperty("carga.ivs.col"));
			int anioMinimo = Integer.parseInt(prop.getProperty("carga.anio"));
			
			String anio = data[0].trim();
			String mes = data[1].trim();
			String actividad = data[2].trim();
			String valor = data[4].trim().replace(".","").replace(",",".");
			...
			// Validar tipo de dato actividad
			} else if (!Util.esNumero(actividad)) {
				Util.setLog(archivoLog, "I", "Insertar Registros --> ERROR en línea " + cuenta, "tipo invalido de actividad");
				bitacora.add("213;" + cuenta + ";actividad;" + actividad + ";Tipo invalido de actividad, línea " + cuenta);
			
			// Validar tipo de dato valor actividad
			} else if (!Util.esDecimal(valor)) {
				Util.setLog(archivoLog, "I", "Insertar Registros --> ERROR en línea " + cuenta, "tipo invalido de valor actividad");
				bitacora.add("213;" + cuenta + ";valor actividad;" + valor + ";Tipo invalido de valor actividad, línea " + cuenta);
				...
				base.setId_actividad(Integer.parseInt(actividad));
				base.setValor(Double.parseDouble(valor));
	...
	private void insertarCarga(long cuenta, int estado, int error) throws Exception {
		...
		try {
			...
			carga.setId_base(20); //IVS
			carga.setId_estado_carga(estado);
			carga.setId_error_carga(error);
			carga.setNum_rows(cuenta);
			carga.setNum_cols(Integer.parseInt(prop.getProperty("carga.ivs.col")));
