...
import cl.sernatur.service.impl.BaseIpcServiceImpl;
...
public class DataIpcNegocio {
	...
	private BaseIpc base;
	private BaseIpcService baseService;
	private Carga carga;
	...
	private void eliminarRegistros() throws Exception {
		baseService = new BaseIpcServiceImpl(con);
		try {
			String desde = formatoFecha[0] + formatoFecha[1];
			String hasta = formatoFecha[2] + formatoFecha[3];
	...
	private boolean validartDatos(String[] data, long cuenta) throws Exception {
		...
		try {
			...
			// Validar tipo de dato servicio
			} else if (!Util.esNumero(servicio)) {
				Util.setLog(archivoLog, "I", "Insertar Registros --> ERROR en línea " + cuenta, "tipo invalido de servicio");
				bitacora.add("213;" + cuenta + ";servicio;" + servicio + ";Tipo invalido de servicio, línea " + cuenta);
			
			// Validar tipo de dato valor servicio
			} else if (!Util.esDecimal(valor)) {
				Util.setLog(archivoLog, "I", "Insertar Registros --> ERROR en línea " + cuenta, "tipo invalido de valor servicio");
				bitacora.add("213;" + cuenta + ";valor servicio;" + valor + ";Tipo invalido de valor servicio, línea " + cuenta);
				...
				base.setId_servicio(Integer.parseInt(servicio));
	...
	private void insertarCarga(long cuenta, int estado, int error) throws Exception {
		...
		try {
			...
			carga.setId_base(19); //IPC
			carga.setId_estado_carga(estado);
			carga.setId_error_carga(error);
			carga.setNum_rows(cuenta);
			carga.setNum_cols(Integer.parseInt(prop.getProperty("carga.ipc.col")));
